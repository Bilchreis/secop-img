...
\pgfsetlayers{bbackground layer,background layer,main,foreground layer}
	...
	\begin{tikzpicture}[auto, >=triangle 45]
		...
		node [module,below = of nodes, xshift = -1.5cm] (modules) {Modules}
		
		node [property,right = of modules] (node_properties) {Properties}
		
		
		node [command, below  = of modules, yshift = -1.4cm, xshift = -1.2cm,fill = \devcol] (commands){Commands}
		
		node [command, left  = of commands] (parameters){Parameters}
		
		
		
		node [property,below = of commands,fill = black!5] (cmd_properties) {Properties}
		
		node [property,below = of parameters,fill = black!5] (param_properties) {Properties}
		;
		
		\node [acessibles,fill = black!15,above = of parameters, yshift = 0.7cm,font = \Large] (accs){Accessibles};
		
		\begin{pgfonlayer}{background layer}
			%\draw[-,consists] (accs) -- (mod1);
			\node [block, fill= black!15,thick,inner sep=1em,fit= {(accs)(param_properties)(cmd_properties) (commands) (parameters)}] (accs_block) {};
		\end{pgfonlayer}
