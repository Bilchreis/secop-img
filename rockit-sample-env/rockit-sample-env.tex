...
\pgfsetlayers{bbackground layer,background layer,main,foreground layer}
	...
	% Ophyd
	node[archobject,below = of bluesky, yshift = -0.5cm](dosing_ophy){Gas Dosing\\\tiny Ophyd Device}
	...
	\begin{pgfonlayer}{background layer}
		% Sample Environment
		\node[class,fill = yellow!10,fit = {(spectr) (reactor_cell) (temp_contr)}](sample_env){};
		
		% Beamline
		\node[class,fill = red!10,fit = {(epics) (eiger) }](beam){};
		
		
		\node[class,fill = blue!10,fit = {(bluesky) (dosing_ophy) (reactor_ophy) (beamline_ophy)(plan)(doc)(beamline_ophy3)}](bluesky_env){};
		% Bluesky environment
	...
	\end{pgfonlayer}
		...
		\node [box,inner sep=2em,line width=0.3mm,fit = {(sample_env)(beam)(bluesky_env)(ecs)}]	(all){};
\end{pgfonlayer}	
	...
	\draw[hline] ([yshift=0.3cm] field.north west) -- ([yshift=0.3cm,xshift = 12.7cm] field.north west);
	
	\draw[hline] ( [yshift=0.3cm] hard.north west) -- ([yshift=0.3cm,xshift = 12.7cm] hard.north west);
	
	\draw[hline] ([xshift = -0.2cm,yshift= 0.5cm]ecs.north east) -- ([xshift = -0.3cm]hard.south east);
